...
Injector.prototype.invoke = function(fn, args){
    ...
    if(!fn.$get) {
        console.log("Warning: function passed to invoke has no $get constructor.");
        return fn(args);
    }
...
Injector.prototype.inject = function(aFunction, args){
    ...
    if(aFunction.$inject){
        aFunction.$inject.forEach(function(dependencyName){
            ...
            dependencies.push(this.moduleMap[dependencyName]);

        }.bind(this));
    }

    var injectedFunction = aFunction;
    dependencies.forEach(function(dependency){
        var injectedDependency = this.inject(dependency);
        //injectedDependency.$get = dependency.$get;
        var concreteValue = this.invoke(injectedDependency);
        injectedFunction = injectedFunction.bind(injectedFunction, concreteValue);
    }.bind(this));

    if(typeof injectedFunction == "function"){
        injectedFunction.$get = aFunction.$get;
        injectedFunction.$inject = aFunction.$inject;
    }
