...
Injector.prototype.invokeSync = function(fn, self, localArgs){
    ...
    fn.$inject = fn.$inject.map(function(dependency){
       return Q.when(dependency);
    });
    ...
    Q.all(fn.$inject).then(function(args){
        ...
        //fn.$inject = args;
        //this.inject(fn, localArgs);     //reject with resolved dependencies
...
Injector.prototype.bind = function(fn, self, localArgs){
    ...
    /*var bindFunction = function(){
        ...
        //return injectAndBind.apply(this, arguments);
    }.bind(this);*/

    /*bindFunction.rebind = function(){
        fn = this.annotate(originalFn);
        fn = this.inject(fn, localArgs, {ignoreMissing: true});
    }.bind(this);
*/
    //fn = this.inject(fn, localArgs, {ignoreMissing: true});
...
//    someFunction = someFunction.bind(this, fn, self, localArgs);

    someFunction.rebind = function(){
        fn = originalFn;
        //fn = this.inject(originalFn, localArgs, {ignoreMissing: true});
    }.bind(this);
...
/**
 * Injects a function, reading its dependencies from $inject and using injector.get to get all of them
...
Injector.prototype.inject = function(aFunction, localArgs, options){
    ...
    if(aFunction.$inject){
        aFunction.$inject.forEach(function(dependencyName, index){
            aFunction.$inject[index] = this.get(dependencyName);
            if(typeof aFunction.$inject[index] == "undefined") aFunction.$inject[index] = localArgs[dependencyName];
